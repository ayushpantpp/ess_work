...
class EmpDetailComponent extends Component {
    ...
    public function getTravelAmtByDeptDesig($deptCode = NULL, $desigCode = NULL, $city_type = NULL) {
        $tv = $this->TravelMaster->find('first', array(
            'fields' => array('amount'),
            'conditions' => array(
        ...
        return $tv['TravelMaster']['amount'];
    }
        
     public function getTravelHtlAmtByDeptDesig($deptCode = NULL, $desigCode = NULL, $city_type = NULL) {

        $tv = $this->TravelMaster->find('first', array(
            'fields' => array('hotel_amount'),
            'conditions' => array(
                'department_id' => $deptCode,
                'designation_id' => $desigCode,
                'city_type'=>$city_type)
        ));
        return $tv['TravelMaster']['hotel_amount'];
    }
    ...
    public function leaveConditions($leave_code, $tot, $emp_code, $applydate) {
        ...
        //Leave code dynamic starts here
        ...
        $time = strtotime($date);
        $year = date("Y", $time);
        $cl_penfull = $leaveDetail->find('count', array('fields' => array('leave_id'), 'conditions' => array('year(leave_date)' => $year, 'emp_code' => $emp_code, 'leave_status' => array(5, 2, 7), 'leave_code' => $cl_leave_code['LeaveConfiguration']['leave_code'], 'hlfday_leave_chk' => 'N'),
        ));
        $cl_penhalf = $leaveDetail->find('count', array('fields' => array('leave_id'), 'conditions' => array('year(leave_date)' => $year, 'emp_code' => $emp_code, 'leave_status' => array(5, 2, 7), 'leave_code' => $cl_leave_code['LeaveConfiguration']['leave_code'], 'hlfday_leave_chk' => 'Y'),
        ));
        $half = $cl_penhalf / 2;
        ...
        $ocassion = $leaveDetail->find('count', array('fields' => array('leave_id'), 'conditions' => array('year(leave_date)' => $year, 'emp_code' => $emp_code, 'leave_status' => array(5, 2, 7), 'leave_code' => $ol_leave_code['LeaveConfiguration']['leave_code']),
        ...
        switch ($leave_type['LeaveConfiguration']['leave_type']) {
            case 'EL':
                ...
                $leavecheckcasualafter = $leaveDetail->find('all', array('fields' => array('leave_id', 'leave_code', 'Count(leave_id)'), 'conditions' => array('leave_date' => $datetime1, 'emp_code' => $emp_code, 'leave_status' => array(5, 2, 7), 'leave_code' => $cl_leave_code['LeaveConfiguration']['leave_code']), 'group' => array('leave_id')));
                if (count($leavecheckcasualafter) > 0) {
                    $lv_app = $this->LeaveDetail->updateAll(
                            array('LeaveDetail.leave_code' => $cl_leave_code['LeaveConfiguration']['leave_code']), array('LeaveDetail.leave_id' => $leavecheckcasualafter[0]['LeaveDetail']['leave_id'])
                    );
                }
            ...
            case 'CL':
                ...
                $datetime1 = date('Y-m-d', strtotime($leave_start_date . ' -1 day'));
                $datetime2 = date('Y-m-d', strtotime($applydate . ' +1 day'));
                $leavecheckbefore = $leaveDetail->find('first', array('fields' => array('leave_id'), 'conditions' => array('leave_date' => $datetime1,
                        'leave_code' => $cl_leave_code['LeaveConfiguration']['leave_code'], 'emp_code' => $emp_code, 'leave_status' => array(5, 2, 7))));
                  $leavecheckbefore_count = $leaveDetail->find('count', array('conditions' => array('leave_id' => $leavecheckbefore['LeaveDetail']['leave_id'])));
				  $leavecheckafter = $leaveDetail->find('first', array('fields' => array('leave_id'), 'conditions' => array('leave_date' => $datetime2,
                        'leave_code' => $cl_leave_code['LeaveConfiguration']['leave_code'], 'emp_code' => $emp_code, 'leave_status' => array(5, 2, 7))));
                  $leavecheckafter_count = $leaveDetail->find('count', array('conditions' => array('leave_id' => $leavecheckafter['LeaveDetail']['leave_id'])));
                 $totalleavecount = $leavecheckafter_count + $leavecheckbefore_count + $tot;
				$query = $leaveDetail->find('count', array('fields' => array('leave_id'), 'conditions' => array('year(leave_date)' => $year, 'emp_code' => $emp_code, 'leave_status' => array(5, 2, 7), 'leave_code' => $cl_leave_code['LeaveConfiguration']['leave_code'], 'hlfday_leave_chk' => 'N')));
				 if ($query < 12) {
                    ...
                    if ($leave_total > 12) {
                        ...
                        if ($totalleavecount > 3) {
                            $msg = 'You can avail Maximum 3 Consicutive Casual Leaves';
                            return $msg;
                        } else {
                            return true;
                        }
            ...
            case 'ML':
                ...
                if ($leavecheckcasual > 0 || $leavecheckcasualbefore > 0) {
                    $msg = 'ML cannot be combined with casual Leave';
                    return $msg;
                }
                $query = $leaveDetail->find('count', array('fields' => array('leave_id'), 'conditions' => array('year(leave_date)' => $year, 'emp_code' => $emp_code, 'leave_status' => array(5, 2, 7), 'leave_code' => $ml_leave_code['LeaveConfiguration']['leave_code'])));

                $ocassion = $leaveDetail->find('count', array('fields' => array('leave_id'), 'conditions' => array('emp_code' => $emp_code, 'leave_status' => array(5, 2, 7), 'leave_code' => $ml_leave_code['LeaveConfiguration']['leave_code']),
                    'group' => array('leave_id')));
                if ($ocassion > 2) {
            ...
            case 'PL':
                ...
                $leavecheckcasualbefore = $leaveDetail->find('count', array('fields' => array('leave_id'), 'conditions' => array('leave_date' => $datetime2, 'emp_code' => $emp_code, 'leave_status' => array(5, 2, 7), 'leave_code' => $cl_leave_code['LeaveConfiguration']['leave_code'])));
                if ($leavecheckcasual > 0 || $leavecheckcasualbefore > 0) {
                    $msg = 'PL cannot be combined with casual Leave';
                    return $msg;
                }
                $query = $leaveDetail->find('count', array('fields' => array('leave_id'), 'conditions' => array('year(leave_date)' => $year, 'emp_code' => $emp_code, 'leave_status' => array(5, 2, 7), 'leave_code' => $pl_leave_code['LeaveConfiguration']['leave_code'])));
                $leave_total = $query + $tot;
                if ($leave_total > 30) {
                    $msg = 'You cannot avail Maternity Leave for more than 30 days';
                    return $msg;
                } else {
                    return true;
                }
    ...
    public function editLeaveConditions($leave_code, $tot, $emp_code, $applydate, $leaveid) {
        ...
        switch ($leave_type['LeaveConfiguration']['leave_type']) {
            ...
            case 'CL':
                ...
                $datetime1 = date('Y-m-d', strtotime($leave_start_date . ' -1 day'));
                $datetime2 = date('Y-m-d', strtotime($applydate . ' +1 day'));
                $leavecheckafter = $leaveDetail->find('all', array('fields' => array('leave_id', 'leave_code', 'Count(leave_id)'), 'conditions' => array('leave_date' => $datetime1, 'emp_code' => $emp_code, 'leave_status' => array(5, 2, 7))));
                $totleavebefore = $leaveDetail->find('all', array('fields' => array('leave_id', 'leave_code', 'Count(leave_id) tot'), 'conditions' => array('leave_id' => $leavecheckafter[0]['LeaveDetail']['leave_id'], 'emp_code' => $emp_code, 'leave_status' => array(5, 2, 7)), 'group' => array('leave_id')));
                $totalleavecountbefore = $totleavebefore[0][0]['tot'] + $tot;
                $query = $leaveDetail->find('count', array('fields' => array('leave_id'), 'conditions' => array('year(leave_date)' => $year, 'emp_code' => $emp_code, 'leave_status' => array(5, 2, 7), 'leave_code' => $cl_leave_code['LeaveConfiguration']['leave_code'], 'hlfday_leave_chk' => 'N')));
                if ($query < 12) {
                    $leave_total = $query + $tot;
                    if ($leave_total > 12) {
            ...
            case 'ML':
                ...
                $leave_total = $query + $tot;
                if ($leave_total > 30) {
                    $msg = 'You cannot avail Parentry Leave for more than 30 days ';
                    return $msg;
                } else {
                    return true;
                }
    ...
    public function getEmpEmailDetails($empCode, $compCode) {
        ...
        $emplist = $model->find('first', array(
            'fields' => array('UserDetail.emp_code', 'UserDetail.email', 'UserDetail.comp_code', 'UserDetail.user_name'),
    ...
    public function getEmpDetails($empCode) {
        ...
        $emplist = $model->find('first', array(
            'fields' => array('MyProfile.emp_code', 'MyProfile.manager_code', 'MyProfile.emp_full_name', 'MyProfile.join_date', 'MyProfile.location_code', 'MyProfile.dept_code', 'MyProfile.desg_code', 'MyProfile.comp_code', 'MyProfile.contact'),
    ...
    public function checkApplicationDelay($app_id , $org_id){
        
        $app_constrain = new Model(array('table' => 'application_constrains', 
            'ds' => 'default', 'name' => 'APPCONSTRAIN'));
        $data = $app_constrain->find('first', array('fields' => array('apply_in_days'), 
            'conditions' => array('app_id' => $app_id,'org_id'=>$org_id)));
        
        return $data['APPCONSTRAIN']['apply_in_days'];
