...
//put your code herei
	...
	$this->autoRender = false;
        ...
        foreach ($data_oracle as $k => $v) {
            ...
            $data_user['user_password'] = $this->Auth->password(str_replace('-','',date('d-m-Y', strtotime($data['dob']))));
...
//create data for user table
                        ...
                        $data_user['user_password'] = $this->Auth->password(str_replace('-','',date('d-m-Y', strtotime($v['OracleHcmEmpPrf']['dob']))));
                        $data_user['user_name'] = $data['emp_firstname'];
                        if (isset($data['emp_lastname'])) {
                            $data_user['user_name'] .= '.' . $data['emp_lastname'];
                        }
...
//take that value from oracle
                        $row = $this->OraHcmEmpLeaveEncsh->find('first', array(
                            'conditions' => $oracle_cond
                        ));
                        //insert that value in mysql replica table
...
//insert/update the value inserted in replica table to original table
...
//exit();
//$this->perform_operation('LeaveEncash', null, $row, $val['OraDataInfo']['operation']);
...
//disable data sync
    ...
    function integration_update_salary() {
        ...
        if ($syn == 0) {
            ...
            //dont update the values if present in newdata
            //truncate oracle's database's table if possible
            // $this->GeneralOracle->query('DELETE DATA_INFO');
            //enable all triggers
            $this->General->query('SET @DISABLE_TRIGGERS = NULL;');
            //disable data sync
    ...
    function updatepass() {
        ...
        foreach ($result as $pass) {
            ...
            //$encryptpass = hash('sha512',$newDate);
