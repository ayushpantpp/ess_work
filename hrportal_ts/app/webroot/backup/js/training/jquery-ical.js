    ...
    $.fn.ical = function(options){
        ...
        */
        function createNavigation(obj, insdate){
            obj.append("<div style='text-align:center;'><span id = 'currentmonth'>"+ options.monthnames[insdate.getMonth()] +"</span> <span id = 'currentyear'>"+ insdate.getFullYear() +"</span></div>" +
                      "<div style='text-align:center;'><span id ='icalprev'><</span><span id ='icalnext'>></span></div>");
            ...
            for (var i = 1; i <= days; i++) //each day in month
            {
                ...
                else
                {
					...
					for (var k = 0; k < jsondates.length; k++)
					{
						...
						if (firstEvent)
						{
							$(".icaltable tr:last", obj).append("<td onclick='showtraining(this.id)' class='date_has_event " + datejson.className + "' id = '"+ formatdate +"'>"+i+"<div class='events'><ul id ='ul-"+ formatdate +"'><li><div class='dbl' id='dblArrw' style='top:53px;'>&nbsp;</div></li>"+ str +"</ul></div></td>"); //add day  
							firstEvent = false;
						}
						else
						{
