...
<div id="page_content" role="main">
    <div id="page_content_inner">
        ...
        <div class="md-card">
            <div class="md-card-content large-padding">
                    ...
                    <tbody>
                     <?php $i = 0;
                        ...
                        <tr class="<?php echo $class; ?>">
                            ...
                            <td ><strong> <?php if (!empty($v['LeaveWorkflow']['fw_date'])) { echo date('d-M-Y', strtotime($v['LeaveWorkflow']['fw_date']));
                                                } else { echo date("d-M-Y");} ?>
                            </strong> </td>
                            <td><strong><?php if (!empty($v['LeaveWorkflow']['remark'])) {
                                                    echo $v['LeaveWorkflow']['remark']; ?>
                            <?php } else echo "N/A" ?></strong></td>
                        <tr>    
                    <?php $i++; } } ?> 
                        <tr>
                            <td colspan="3" style="display:none"> 
                                <input type="hidden" value ="<?php echo $leave; ?>" name="data[LeaveWorkflow][leave_id]"> 
                                <input type="hidden" value ="<?php echo $LeaveWorkflowid[0][0]['leave_wf_id']; ?>" name="data[LeaveWorkflow][id]">
                            </td>
                        <tr>
                                <?php
                                $deptcode = $this->Common->getemocodebydept($getlvl[0]['LeaveWorkflow']['emp_code']);
                                echo $checklvlapp = $this->Common->findAppLevel($appId);
                                echo $lvl = count($getlevel);
				if (count($getlevel) == $checklvlapp) {
                                ...
                                $checkForwardHr = $this->Common->checkForwardHr(2);?>
                        <tr class="hidehr"> 

                        <?php if ($lvl == $checklvlapp && !$checkForwardHr) { ?>
                            <td><input type="radio" class="flat" name="data[LeaveWorkflow][type]" value="5" checked="checked" onclick="displaytype(this.value);"> <strong>Approve </strong></td>
                        <?php } else { if($lvl > $checklvlapp){ ?>
                               <td><input type="radio" class="flat" name="data[LeaveWorkflow][type]" value="5" checked="checked" onclick="displaytype(this.value);"> <strong>Approve </strong></td>
                        <?php } else { ?>
                        <?php if ($lvl < $checklvlapp) { ?>
                        ...
                        <?php } else if ($lvl == $checklvlapp && $checkForwardHr) { ?>
                             <td><input type="radio" class="flat" name="data[LeaveWorkflow][type]" value="6" checked="checked" onclick="displaytype(this.value);" ><strong> Forward </strong>
                            </td>      
                          
                            
                       <?php } } }?>
                        <?php if ($checkrevert) { ?>
                            <!--<td><input type="radio" class=" flat" name="data[LeaveWorkflow][type]" value="3" onclick="displaytype(this.value);"><strong> Revert </strong></td> -->
                        <?php } ?>
                            <td colspan="3"><input type="radio"  class="form-control flat" name="data[LeaveWorkflow][type]" onclick="displaytype(this.value);" value="4"><strong> Reject </strong></td>
                        </tr>
                        <tr id="reject" style="display:none;">
                            <td align="right">Remark*</td>
                            <td colspan="5">
                                <table>
                                    <tr><td><textarea class="form-control" style="width: 348px; height: 95px;"  name="data[LeaveWorkflow][reject_remark]" id="rejcmnt" col="100" row="100"  ></textarea></td>
                                    </tr>
                                </table>
                            </td>
                        </tr> 
        <?php if ($checkrevert) { $revertemplist = $this->Common->getRevertEmp($leave); ?>
                        <tr id="revert" style="display:none;">
                            <td align="right">Revert*</td>
                            <td colspan="5">
                                <table>
                                    <tr><td><?php echo $this->Form->input('LeaveWorkflow.revert_emp_code', array('type' => 'select', 'label' => false, 'options' => $revertemplist, 'class' => 'form-control round_select', 'id' => 'revertempcode')); ?> 
                                        <tr>
        ...
        <?php } ?>      
        <?php if ($lvl < $checklvlapp && $leaveStatus != 6) { ?>
        ...
        <?php } else { ?>
            <?php if ($lvl == $checklvlapp && $checkForwardHr) {
                        ...
                        <tr id="forward_hr_tr" class="hidehr">          
                            ...
                            <td colspan="2">
                                <table><tr><td><?php echo $this->Form->input('LeaveWorkflow.hr_emp_code', array('type' => 'select', 'label' => false, 'empty' => '- -Select- -', 'options' => $hrList, 'class' => 'form-control round_select', 'id' => 'fwhrempcode')); ?>
                        ...
                        <tr id="approved_tr" class="hidehr">
                            <td align="right">Remark :</td>
                            <td colspan="5">
                                <table><tr><td ><textarea class="form-control" style="width: 348px; height: 95px;" name="data[LeaveWorkflow][approve_remark]" id="appcmnt" col="100" row="100" ></textarea></td>
                                    </tr>
                                </table></tr>       
        <?php } ?>
    <?php } ?>  </tbody>
                </table><?php echo $this->Form->submit('APPROVE', array('onClick' => 'return checkSubmit()', 'name' => 'data[LeaveWorkflow][save]', 'class' => 'md-btn md-btn-success')); ?>
    <?php }
     echo $this->Form->input('LeaveWorkflow.leave_type', array('type' => 'hidden','value'=>$leavetype, 'class' => 'form-control round_select', 'id' => 'leave_code'));
