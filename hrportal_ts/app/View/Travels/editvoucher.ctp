            ...
            <div class="uk-grid" data-uk-grid-margin> 
                <div class="uk-width-medium-1-2 ">
                        <div class="parsley-row">
                            ...
                            <?php 
                            echo $this->form->input('Voucher.city_group', array('type'=>'select','label'=>false,'onChange' => "getCityPrice(this.value)",'required'=>true,'class'=>"data-md-selectize",'options'=>$city_grp_list,'default'=>$travel_info['DtTravelVoucher']['place_visit']));
                            ?>
                        </div>
                    </div>
                ...
                <div class="uk-width-medium-1-2">                                
                    <div class="parsley-row">  
                        <label for="req_ref">Department <span class="req">*</span></label>
                          <?php  echo $this->Form->input('Voucher.EmpDepartment', array('type' => 'text', 'label' => false,  'value'=> $this->Common->getdepartmentbyid($dept),'readonly'=>'redaonly', 'class' => 'md-input')); ?>
                        
                            <?php echo $this->Form->input('Voucher.Emp_desg', array('type' => 'hidden', 'value' => $designation_code)); ?>
                            <?php echo $this->Form->input('Voucher.Emp_department_code', array('type' => 'hidden', 'value' => $dept)); ?>
                    </div>
                </div>
...
<?php echo $this->Form->input('Voucher.Conveyence_amt', array('label' => false, 'type' => 'text', 'class' => 'md-input', 'onKeyup' => 'calTotal()', 'MAXLENGTH' => '9', 'value'=> $travel_info['DtTravelVoucher']['conv_expense'])); ?>
                ...
                <div class="uk-width-medium-1-3">
						...
						<?php foreach($local_expense_dtl as $led) {?>

                                                    <tr>
                                                        <td class="uk-text-center uk-width-small-1-10">1<?php echo $this->Form->input("rowCount", array("type" => "hidden","id"=>"rowCount","value"=>"1")); ?></td>                            
                                                        <td><?php echo $this->Form->input('Voucher.local_claim_date.', array('class' => 'md-input LocalClaimDate','id'=>'ayush1', 'value'=>$led['LocalExpense']['local_claim_date'],"data-uk-datepicker"=>"{format:'DD-MM-YYYY',maxDate:'".date('d-m-Y')."'}")); ?></td>
                                                        <td><?php echo $this->Form->input('Voucher.local_claim_mode.', array('class' => 'md-input','type'=>'select','options'=>$Going_by, 'default'=>$led['LocalExpense']['local_claim_mode'])); ?></td>
                                                        <td><?php echo $this->Form->input('Voucher.local_claim_amount.', array('class' => 'md-input amt_local','onBlur'=>'calTotal()', 'value'=>$led['LocalExpense']['local_claim_amount'])); ?></td>
                                                 </tr>
						<?php } ?>
                                                </table>
            ...
            <div class="uk-grid uk-grid-width-large-1-3 uk-grid-width-xlarge-1-6" data-uk-grid-margin>
                ...
                <?php } else { ?>
                    ...
                    <div class="parsley-row">
                        <select id="daily_days" name="data[Voucher][days]" class ="md-input"  onKeyup="tot_allow()"  onChange="tot_allow()"  onBlur="return calcDate()">
                            <?php foreach($option_days as $oppRec){
                                if($DA_Days==$oppRec){
                                    echo "<option value='".$oppRec."' selected='selected'>".$oppRec."</option>";
                                }else{
                                echo "<option value='".$oppRec."'>".$oppRec."</option>";
                                }
                            }
                            ?>
                        </select>
            ...
            <div class="uk-grid uk-grid-width-large-1-3 uk-grid-width-xlarge-1-6" data-uk-grid-margin>
			    ...
			    <div class="parsley-row">
                            <select id="daily_dayss"  name="data[Voucher][hotel_days]" class="md-input" required="required"  onKeyup="tot_allows()" onChange="tot_allows()"  onBlur="return calcDate()">
				<?php foreach($option_days as $oppRec){
                                if($DA_Days==$oppRec){
                                    echo "<option value='".$oppRec."' selected='selected'>".$oppRec."</option>";
                                }else{
                            ...
                            <div class="parsley-row float-left">
                                <input type="text" id="hotel_amt" class ="md-input" onkeyup="tot_allow()" value = "<?php echo $hotel_amount; ?>" readonly="readonly"/>
                            </div>
                            <div class="parsley-row">
                                per Day :   
                            </div>
