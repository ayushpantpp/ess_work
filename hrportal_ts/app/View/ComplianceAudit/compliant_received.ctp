...
function ShowDet(val){ 
        ...
        var ceo_res = 'ceo_resp';
        $.ajax({
            type: "POST",
            url: '<?php echo $this->webroot ?>ComplianceAudit/show_details/' + val+'/'+ceo_res,
            //data:'project_id='+val,
...
//                            print_r($InvestCompliants);
                            for($i=0;$i<count($InvestCompliants);$i++)
                      {
                            ...
                            <tr>
                                ...
                                <td><span class="uk-text-small uk-text-muted uk-text-nowrap">
                                       <?php if($InvestCompliants[$i]['CAInvestigation']['current_status'] == '0'){?>
                                        <a class="uk-badge uk-badge-success" href="<?php echo $this->Html->url('/ComplianceAudit/compliant_response_send/' . base64_encode($InvestCompliants[$i]['CAInvestigation']['id'])); ?>">Send Response</a>
                                       <?php }else{ ?>
