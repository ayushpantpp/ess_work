<script>
    ...
    function getDataTypeForm(val) {
        window.location.replace("<?php echo $this->webroot ?>ComplianceAudit/audit_entry/" + val);
    }

    function ShowDet(val) {
        $.ajax({
            type: "POST",
            url: '<?php echo $this->webroot ?>ComplianceAudit/audit_entry/' + val,
            //data:'project_id='+val,
        ...
        <div class="md-card" >
            <div class="md-card-content ">
                <div class="uk-grid" data-uk-grid-margin>
                    ...
                    <div class="uk-width-medium-1-3" >
                        <div class="parsley-row">
                            <a  class="md-btn md-btn-primary uk-float-right" href="<?php echo $this->Html->url('audit_entry_save'); ?>">Audit Entry</a>
                        </div>
                    </div>
                </div>
                            ...
                            <table class="uk-table uk-table-align-vertical uk-table-nowrap tablesorter tablesorter-altair" id="ts_issues">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="uk-text-center">Audit Parameter</th>
                                        <th class="uk-text-center">Entry Date</th>
                                        <?php
                                        foreach ($labels as $key => $val) {
                                            ?>
                                            <th class="uk-text-center"><?php echo ucfirst($val); ?></th>
                                        <?php } ?>
                                    </tr>
                                </thead>
                                <tbody>
