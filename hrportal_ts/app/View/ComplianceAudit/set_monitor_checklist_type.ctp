    ...
    <div id="page_content_inner" >
        ...
        <div class="md-card uk-width-medium-1-1"  id="addform"  style="<?php echo $display; ?>">  
            <div class="md-card-content">
                ...
                <?php echo $this->Form->create('frm', array('url' => array('controller' => 'ComplianceAudit', 'action' => 'set_monitor_checklist_type'), 'type' => 'file', 'id' => 'form_validation', 'class' => 'uk-form-stacked padding-left-lg')); ?>
                            ...
                            for ($i = 0; $i < count($setType); $i++) {
                                ...
                                <tr>
                                    ...
                                    <td><span class="uk-text-small uk-text-muted uk-text-nowrap">
                                            <a class="uk-badge uk-badge-success" id="form_open" href="<?php echo $this->Html->url('set_monitor_checklist_type/' . $setType[$i]['CASetChecklistTypeAuditMonitoring']['id']); ?>">Edit</a>
                                            | 
                                            <?php if($setType[$i]['CASetChecklistTypeAuditMonitoring']['status'] == '0'){ ?><a class="uk-badge uk-badge-danger" id="form_open" href="<?php echo $this->Html->url('set_monitor_checklist_type/'.$setType[$i]['CASetChecklistTypeAuditMonitoring']['id'].'/1');  ?>" onclick="return confirm('Are you sure?');">Inactive</a></span>
                                         <?php }else{ ?><a class="uk-badge uk-badge-info" id="form_open" href="<?php echo $this->Html->url('set_monitor_checklist_type/'.$setType[$i]['CASetChecklistTypeAuditMonitoring']['id'].'/0');  ?>" onclick="return confirm('Are you sure?');">Active</a></span>
                                         <?php }?>
                                         </td>
                                </tr>
    <?php $p++;
