<?php $i = 0; ?>
<div id="page_content">
    <div id="page_content_inner">
       
         <?php echo $flash = $this->Session->flash(); ?>
        <div class="md-card">
            <div class="md-card-content">
                <div class="uk-overflow-container uk-margin-bottom">
           
            <?php if ($page_name == 'ATTENDANCE_LIST') { ?>
                <?php
                echo $this->form->create('Users', array('url' => '', 'name' => 'Form1', 'action' => 'attendance_report', 'inputDefaults' => array('label' => false, 'div' => false, 'error' => array('wrap' => 'span', 'class' => 'md-input'))));
                ?>
                 <table class="uk-table uk-table-align-vertical uk-table-nowrap tablesorter tablesorter-altair" id="ts_pager_filter">
                     <tr> 
                         <td><label  class="uk-form-label">Select Month</label><?php echo $this->form->input('month', array('label' => false, 'type' => 'select', 'options' => $months, 'value' => $current_month, 'class' => "md-input", 'required' => true, 'id' => 'first_name')); ?></td>
                         <td>
 <label  class="uk-form-label">Select Year</label>
                         	<?php echo $this->form->input('year', array('label' => false, 'type' => 'select', 'options' => $years, 'value' => $current_year, 'class' => "md-input", 'required' => true)); ?></td>
<td >
                           <label for="kUI_multiselect_basic" class="uk-form-label">Select Employee</label>
                             <?php
                              $emplist=$this->Common->getatEMPist();

                            echo $this->form->input('Employee', array('label' => false,'type' =>'select','multiple'=>'multiple', 'id'=>'kUI_multiselect_basic','empty' => ' -- Select --', 'options' => $emplist));
                            ?>



                        </td>

                         <td><input type="submit" class="md-btn md-btn-primary" value="GO" name='post_Salary'></td>
                         <td><input type="submit" class="md-btn md-btn-danger" value="EXPORT" name='post_Salary'></td>
                    </tr>
                 </table>    
                </form>
            <?php } ?>
            <div class="clearfix"></div>
        </div>
                </div></div>
        <div class="md-card">
        	 
        <div class="md-card-toolbar">
          

                          
                             
                            <h3 class="md-card-toolbar-heading-text">
                              <b> Attendance List</b>
                            </h3>
                            
                          

                        
                            </div>
            <div class="md-card-content">
                <div class="uk-overflow-container uk-margin-bottom">

                    <table class="uk-table uk-table-align-vertical uk-table-nowrap tablesorter tablesorter-altair" id="dt_colVis">
                        <thead>
                            <tr>                
                                <th>Sr.No</th>
                                <th>Employee Name</th>
                                <th>Attendance Date</th>
                                <th>Applied Date</th>
                                <th>In Time</th>
                                <th>Out Time</th>
                                <th>Status</th>
                                <th>Remark</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php if (empty($pen_attendances)) { ?>
                                <tr class="even pointer">
                                    <td style="text-align:center;" colspan="11">
                                        <em>--No Records Found--</em>
                                    </td>
                                </tr>
                            <?php } ?>

                            <?php foreach ($pen_attendances as $srcdet) {
                                if ($i % 2 == 0)
                                    $class = 'even pointer';
                                else
                                    $class = 'odd pointer';
                                ?>
                                <tr class="even pointer">
                                    <td><?php echo $i + 1; ?></td> 
                                    <td> <?php echo $this->Common->getempname_bydoc_id($srcdet['AttendanceDetail']['emp_doc_id']); ?></td>
                                    <td> <?php echo date('d-m-Y', strtotime($srcdet['AttendanceDetail']['atten_dt'])) ?></td>
                                    <td> <?php echo date('d-m-Y', strtotime($srcdet['AttendanceDetail']['usr_id_create_dt'])) ?></td>
                                    <td> <?php echo date('H:i', strtotime($srcdet['AttendanceDetail']['in_time'])); ?></td>
                                    <td> <?php echo date('H:i', strtotime($srcdet['AttendanceDetail']['out_time'])); ?></td>
                                    <td> <?php echo $this->Common->findSatus($srcdet['AttendanceDetail']['status']); ?></td>
                                    <?php if ($srcdet['AttendanceDetail']['reject_remark']) { ?>
                                        <td> <?php echo $srcdet['AttendanceDetail']['reject_remark'] ?></td>
                                <?php } else { ?>
                                        <td><p>N/A</p></td>
    <?php } ?>
                                </tr>
    <?php $i++;
} ?>
                        </tbody>
                    </table>
                </div>
                <div class="clearfix"></div>
                <div class="uk-grid" data-uk-grid-margin>
                    <div class="uk-width-medium-1-1">
                        <ul class="uk-pagination uk-pagination-right">
<?php
/*  echo $this->Paginator->prev('Previous', array('tag' => 'li', 'escape' => false), '<a>Previous</a>', array('class' => 'uk-disabled', 'tag' => 'li', 'escape' => false));
echo $this->Paginator->numbers(array('separator' => '', 'tag' => 'li', 'currentLink' => true, 'currentClass' => 'uk-active', 'currentTag' => 'a'));
echo $this->Paginator->next('Next', array('tag' => 'li', 'escape' => false), '<a>Next</a>', array('class' => 'uk-disabled', 'tag' => 'li', 'escape' => false));
 */ ?>
                        </ul>
                    </div>

                </div>

            </div>

            <div id="container" ></div>
        </div>
