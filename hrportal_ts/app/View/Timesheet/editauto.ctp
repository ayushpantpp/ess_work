...
<?php foreach($primary_milestone_list as $key=>$val)
    {$listing_mile .= '<option value='.$key.'>'.$val.'</option>';}?>
    ...
    <div id="page_content">
        ...
        <?php echo $flash = $this->Session->flash(); ?>
                            ...
                            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                ...
                                <tr>
                                    ...
                                    <td><?php echo $this->Common->getempnamebyid($rwTsRec[0]['MstTimesheet']['vc_emp_id']); ?>
                                        ...
                                        <input name="empid" type="hidden" id="empid" value="<?php echo $rwTsRec[0]['MstTimesheet']['vc_emp_id']; ?>"size="4">
            ...
            <div class="md-card"> 
                    ...
                    <div class="uk-overflow-container uk-margin-bottom" >
                        <table border="1" class="uk-table uk-tab-responsive main tbl" id="TextBoxesGroup">
						 ...
						 <?php if (($rwTsRec[0]['MstTimesheet']['vc_status'] == 'P')) { ?>
                                    ...
                                    <th class="filter-false remove sorter-false" style="width:20px !important"><input type="checkbox" name="all[]" id="check_all"/>All </th>
						 <?php } else { ?>
                                    <th class="uk-text-center md-bg-blue-100 uk-te uk-text-small" style="width:20px !important">Check*</th>
                                                 <?php } ?>
				...
				} // print_r($rwTsRec[0]['DtTimesheet']); die;?>

			<?php $x = 0; foreach($rwTsRec[0]['DtTimesheet'] as $rw) { ?>
                            <tr>
                             <?php if (($rwTsRec[0]['MstTimesheet']['vc_status'] == 'P')) { ?>
                                <td><input style="width:20px !important" type="checkbox" name="all[]" class="checkSingle" id="check_<?php echo $x;?>" value = <?php echo $rw['id'];?> <?php echo $rw['vc_cm_code'];?>></input></td><?php } else {?><td></td>
                                <?php } if($rw['vc_cm_code'] == 'Checked'){$value = "disabled";} else {$value = "";} if($rw['tms_id'] == '0'){$tms_id_di = "Others";} else {$tms_id_di = $rw['tms_id'];} ?>
                                <?php $read0nly = ($rw['tms_id'] != "Others") ? '' : ''; ?>
                                <td><?php echo $this->Form->input("record_id.", array("class" => "uk-width-medium-1-1 tags", "style"=>"width:80px !important", "maxlength" => "6", "label" => "", "type" => "hidden", "required" => True,"autocomplete"=> "off",'value'=>$rw['id'],$value)); ?>
                                    <?php echo $this->Form->input("tms_id.", array("class" => "uk-width-medium-1-1 tags", "style"=>"width:45px !important", "maxlength" => "6", "label" => "", "type" => "text","id"=>"tms_id_$x", "required" => True,"extra_ele"=>"$x" ,"autocomplete"=> "off",'value'=>$tms_id_di,$value)); ?>
                                </td>
                                ...
                                <?php echo $this->Form->input("customer_code.", array("class" => "uk-width-medium-1-1", "id" => "customer_code$x", "style"=>"width:45px !important", "maxlength" => "3","type" => "hidden", "required" => True,"readOnly",'value'=>$rw['nu_customer_no'],$value)); ?>
                                <div id="suggesstion-box<?php echo $x;?>"></div></td>
								<?php $mile_sto = $this->Common->getTsActivity($rw['nu_customer_no']); ?>
                                <td><?php echo $this->Form->input("milestone_id.", array("id" => "milestone_id_$x","type" => "select","options"=>$mile_sto,'default'=>$rw['tms_customer'],$value)); ?></td>
                                <td><?php echo $this->Form->input("primary_milestone_id.", array("id" => "primary_milestone_id$x","type" => "select","options"=>$primary_milestone_list,'default'=>$rw['primary_milestone_id'],$value)); ?></td>
			...
			<?php  $x++; }?>
                ...
                <?php } else { ?>
                        ...
                        <input name="Reject" id="reject" type="button" class="md-btn md-btn-primary" value="Reject" data-uk-modal="{target:'#modal_overflow'}"/>
				<?php } ?>
...
<script type="text/javascript">
    ...
    var valu_pri = '<?php echo $listing_mile;?>';
    ...
    $('.plusbtn').click(function () {
        $("#TextBoxesGroup").append('<tr>' +
                ...
                '<td><select name="data[primary_milestone_id][]" class="uk-width-medium-1-1" id="primary_milestone_id_' + counter + '">' + valu_pri +'</select></td>' +
...
<script>
        function get_country(id){
            $.ajax({
                        type: "POST",
                        url: "http://<?php echo $_SERVER["SERVER_NAME"] ?>/hrportal_ts/timesheet/getcustomer/"+id,
        ...
        function get_milestones(id ,ck){
            $.ajax({
                        type: "POST",
                        url: "http://<?php echo $_SERVER["SERVER_NAME"] ?>/hrportal_ts/timesheet/getmilestones/"+id,
                        beforeSend: function () {
                            $("#search-box").css("background", "#FFF url(ajax-loader.gif) no-repeat 165px");
                        },
                        success: function (data) {
							$("#milestone_id_"+ck).html(data);
						}
                    });
        }
...
<div class="uk-width-medium-1-4">
    <div id="modal_overflow" class="uk-modal">
        <div class="uk-modal-dialog uk-modal-dialog-medium">
            ...
            <h4>Reject Remark:</h4>
            <?php echo $this->form->create('timesheet', array('method'=>'post','url'=>'reject_timesheet'));?>
            <?php echo $this->form->input('reject_remark', array('class' => "md-input", 
                          'label'=>false,'type' => 'textarea')); ?>
            <?php echo $this->form->input('timesheet_id', array('class' => "md-input", 
                          'label'=>false,'type' => 'hidden','value'=>$rwTsRec[0]['MstTimesheet']['s_no'])); ?>
            </div>   
            <input name="reject_timesheet" type="submit" class="md-btn md-btn-primary" value="Reject">
