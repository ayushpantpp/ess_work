...
<div> <?php // pr($emp_dtl); die; ?>
    <form id="view" name="view" method="post" action="<?php echo $this->webroot;?>Timesheet/tsview/<?php echo $emp_dtl[0] ?>/<?php echo $emp_dtl[1];?>/<?php echo $emp_dtl[2];?>/<?php echo $emp_dtl[3];?>/<?php echo $emp_dtl[4];?>/<?php echo $emp_dtl[5];?>">
...
?>
			<tr>
				...
				<td style="border-left:#2A1F00 2px solid;border-bottom:#2A1F00 2px solid;" valign="top" align="left"><?php if(!empty($rwTsRec['VC_LEAVE'][$x])){ echo "Yes";}else { echo '-'; }?></td>
...
<?php }?>
		...
		<tr>
			...
			<td width="58%" style="border:#2A1F00 1px solid;height:100px; vertical-align:top;"><b>Signature of Project Manager/Chief Manager/General Manager: <br/> <br/><br/><br /><center><?php if(!empty($rows['MANAGER'][0])){ echo $rows['MANAGER'][0]; }?></center></b></td>
		</tr>
...
<?php if($emp_dtl[4]!='1'){?>
		...
		<tr>
                <td  colspan="10" align="center"><?php if($emp_dtl[4]!= '1' && $emp_dtl[4]!=0 && $emp_dtl[3]!=1){?>
                    <input type="button" id="Print" class="taskbutton" name="Add2" value="Print" onClick="javascript:window.open('<?php echo $this->webroot;?>/timesheet/tsview/<?php echo $emp_dtl[0]?>/<?php echo $emp_dtl[1];?>/<?php  echo $emp_dtl[2]?>/<?php echo $emp_dtl[3]?>/1/<?php echo $emp_dtl[5]?>', '',  'left=2,top=2,titlebar=no,toolbar=no,status=no, width=800 , height=700 , scrollbars=yes');" />&nbsp;
