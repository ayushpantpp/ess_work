...
<div id="add_msg_div">
    <h2 class="demoheaders">Add Leave Type<a href="#" id="create"></a></h2>
    <?php
    echo $this->Form->create('MstLeaveType', array('url' => '#', 'name' => 'msgForm', 'inputDefaults' => array('label' => false, 'div' => false)));
    ?>
        ...
        <div class="input-boxs">
            <table  border="0" width="97%" cellspacing="10" cellpadding="0" align="center" style="margin:0px auto;">
                <tr>
                    ...
                    <th scope="row"><strong>Leave Type Name :</strong>  </th>
                    <td><?php echo $this->Form->input('name', array('class' => 'round_select', 'id' => 'appName','maxLength'=>90)); ?>
                        <div id="dnameErr" style="color:red"></div>
                    </td>
                </tr>
				<tr>
                   
                    <th scope="row"><strong>Leave Type Code :</strong>  </th>
                    <td><?php echo $this->Form->input('leave_code', array('class' => 'round_select', 'id' => 'appCode','maxLength'=>30)); ?>
    ...
    <div id="list_msg_div1">
        <h2 class="demoheaders">Leaves Type List</h2>
    ...
    jQuery(document).ready(function(){
        ...
        jQuery("#delete").live('click',function() {      
            ...
            if(confirm("Are you sure you want to delete?")) {
                jQuery("#overlay").show();
                jQuery.get('<?php echo $this->webroot; ?>LeavesType/delete/'+id,{},function(data) {
                    if(data) {
						jQuery("#overlay").hide();
        ...
        jQuery("#save").live('click', function() {
            ...
            if(appName=='') {
                jQuery("#dcnameErr"+id).html("Enter leave type name.");
                jQuery("#appName"+id).focus();
                err++;
                return false;
            }
			            if(appCode=='') {
                jQuery("#dnameErr"+id).html("Enter leave type code.");
            ...
            if(err==0) {
				...
				jQuery("#overlay").show();
				jQuery("#appCode"+id).removeAttr('disabled');
				jQuery("#appCode"+id).val(appName);
				jQuery("#appCode"+id).attr('disabled',true);
				jQuery("#overlay").show();
                var fdata={'data[MstLeaveType][name]':appName,'data[MstLeaveType][id]':appName1};
                jQuery.post('<?php echo $this->webroot ?>LeavesType/edit/'+id,fdata,function(data) {   
                    if(data) {
                        createMsg(data.msg,data.type);
                        jQuery("#overlay").hide();
        ...
        function lists() {
            ...
            jQuery("#result").html(data);
            var fdata=jQuery("#searchForm").serialize();
            jQuery.post('<?php echo $this->webroot; ?>LeavesType/lists',
				fdata,
				function(data) {
...
function doChk() {
    ...
    if(appName=='') {
        jQuery("#dnameErr").html("<?php echo "Please Enter Leave name."; ?>");
	    ...
	    if(appCode=='') {
        jQuery("#dcodeErr").html("<?php echo "Please Enter Proper leave code."; ?>");
   ...
   if(err==0) {
        ...
        jQuery.post('<?php echo $this->webroot ?>LeavesType/add/',fdata,function(data) {   
        if(data) {
