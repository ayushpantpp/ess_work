        ...
        <div class="input-boxs">
            <table  border="0" width="100%" cellspacing="5" cellpadding="0" align="center" style="margin:0px auto;">
                <tr>
                    ...
                    <td><?php  $company = $this->Common->findCompanyName();
                     echo $this->Form->input('org_id', array('type' => 'select', 'options' => $company, 'class' => 'round_select', 'id' => 'orgName', 'maxLength' => 90)); ?>
                        <div id="" style="color:red"></div>
                    </td>
                </tr>
                <tr>
                    <th scope="row"><strong>Company Name :</strong>  </th>
                    <td><?php echo $this->Form->input('comp_name', array('class' => 'round_select', 'id' => 'appName', 'maxLength' => 90)); ?>
                        <div id="dnameErr" style="color:red"></div>
                    </td>
                </tr>
                <tr>
                    <th scope="row"><strong>Company Alias :</strong>  </th>
                    <td><?php echo $this->Form->input('comp_alias', array('class' => 'round_select', 'id' => 'appName1', 'maxLength' => 90)); ?>
                        <div id="dappnameErr" style="color:red"></div>
                    </td>
                </tr>
                <tr>
                    <th scope="row"><strong>Company Code :</strong>  </th>
                    <td><?php echo $this->Form->input('comp_code', array('class' => 'round_select', 'id' => 'appCode', 'maxLength' => 30)); ?>
...
<script>
    ...
    jQuery(document).ready(function () {
        ...
        jQuery("#delete").live('click', function () {
            ...
            if (confirm("Are you sure you want to delete?")) {
                jQuery("#overlay").show();
                jQuery.get('<?php echo $this->webroot; ?>companies/delete/' + id, {}, function (data) {
                    if (data) {
                        jQuery("#overlay").hide();
    ...
    function lists() {
        ...
        jQuery("#result").html(data);
        var fdata = jQuery("#searchForm").serialize();
        jQuery.post('<?php echo $this->webroot; ?>companies/lists',
                fdata,
                function (data) {
...
<script>
    ...
    function doChk() {
        ...
        if (appName == '') {
            jQuery("#dnameErr").html("<?php echo "Please Enter company name."; ?>");
		 ...
		 if (appName1 == '') {
            jQuery("#dappnameErr").html("<?php echo "Please Enter company alias."; ?>");
            jQuery("#appName1").focus();
            err++;
            return false;
        } else {
            jQuery("#dappnameErr").html("");
        }
        if (appCode == '') {
        ...
        if (err == 0) {
            ...
            jQuery.post('<?php echo $this->webroot ?>companies/add/', fdata, function (data) {
