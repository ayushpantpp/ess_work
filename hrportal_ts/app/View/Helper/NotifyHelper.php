...
class NotifyHelper extends AppHelper {
    ...
    var $helpers = array('Session', 'Common');
    var $name = 'NotifyHelper';
    ...
    public function getSanctionLeave() {
...
*/
        $val = $model->query('SELECT COUNT(DISTINCT(a.leave_id)) AS count FROM leave_workflow a , mst_emp_leaves b, leave_details c 
WHERE a.leave_id = b.leave_id AND b.leave_id = c.leave_id AND a.emp_code = '.$emp_code.' AND a.fw_date IS NULL 
AND a.status IS NULL AND c.leave_status IN (2,6)');
//print_r($val); die;
    ...
    public function getSanctionConv() {
        $emp_code = $_SESSION['Auth']['MyProfile']['emp_code'];
    ...
    public function getSanctionTraining() {
        $emp_code = $_SESSION['Auth']['MyProfile']['emp_code'];
    ...
    public function getSanctionTemp() {
        $emp_code = $_SESSION['Auth']['MyProfile']['emp_code'];
...
//                'fw_date IS NULL',
    ...
    public function getSanctionTravel() {
        $emp_code = $_SESSION['Auth']['MyProfile']['emp_code'];
    ...
    public function pendingTimesheet() {
        $emp_code = $_SESSION['Auth']['MyProfile']['emp_id'];
        App::import("Model", "MstTimesheet");
        $model = new MstTimesheet();
        $val = $model->find('count', array(
            'conditions' => array(
                'vc_emp_id = ' . $emp_code,
                'vc_status' => 'P'
    ...
    public function getPendingConv() {
        $emp_code = $_SESSION['Auth']['MyProfile']['emp_code'];
    ...
    public function getSanctionTimesheet() {
        $emp_code = $_SESSION['Auth']['MyProfile']['emp_code'];
        App::import("Model", "MstTimesheet");
        $model = new MstTimesheet();
        $val = $model->find('count', array(
    ...
    public function pendingTrainingFeedback() {
        $emp_code = $_SESSION['Auth']['MyProfile']['emp_code'];
        App::import("Model", "TrainingCourseAttendence");
        $model = new TrainingCourseAttendence();
        $val = $model->find('all', array(
            'conditions' => array(
                'TrainingCourseAttendence.feedback_status' => 0, 'TrainingCourseAttendence.open' => 1, 'TrainingCourseAttendence.close' => 1, 'TrainingCourseAttendence.trainee_code' => $emp_code, 'curdate() <= DATE_ADD(`TrainingCourseAttendence`.`close_time`,INTERVAL 3 DAY)')
        ));
        return $val;
    }
    ...
    public function pendingTraining() {
        ...
        $val = $model->find('count', array(
            'conditions' => array(
                'TrainingCreation.emp_code' => $emp_code,
                'TrainingCreation.status' => 2)
        ));
        //print_r($val);die;
        return $val;
    }

    public function pendingTrainingAttendence() {
        $emp_code = $_SESSION['Auth']['MyProfile']['emp_code'];
        ...
        $query = $model->find('all', array(
            ...
            'conditions' => array('TrainingScheduleCreation.sch_start_date' => date('Y-m-d'), 'TrainingScheduleCreation.status !=' => 1, 'TrainingCreation.status' => 5, 'TrainingEmployee.trainee_code' => $emp_code, 'TrainingEmployee.status' => 5),
        ));

        App::import("Model", "TrainingConfig");
        $models = new TrainingConfig();
        $trainingConfig = $models->find('first', array('fields' => array('*'), 'conditions' => array('comp_code' => $comp_code)));
        ...
        if (!empty($query)) {
            ...
            foreach ($query as $key => $value) {
                App::import("Model", "TrainingCourseAttendence");
                $attendencemodels = new TrainingCourseAttendence();
                $trainingCourseAttendence = $attendencemodels->find('first', array('fields' => array('*'), 'conditions' => array('trainee_comp_code' => $comp_code, 'trainee_code' => $emp_code, 'training_creation_id' => $value['TrainingCreation']['training_id'])));

                $currenttime = date('Y-m-d H:i:s');
                $time = $value['TrainingScheduleCreation']['sch_start_date'] . " " . $value['TrainingScheduleCreation']['sch_start_time'];
                $time_diff = $trainingConfig['TrainingConfig']['open_attendance_hour'] . "H" . $trainingConfig['TrainingConfig']['open_attendance_min'] . "M";
                $date = new DateTime($time);
                $lasttime = $date->format('Y-m-d H:i:s');
                $date->sub(new DateInterval("PT$time_diff"));
                $time_difference = $date->format('Y-m-d H:i:s');
                $time_difference1[] = $date->format('Y-m-d H:i:s');
                $times[] = $lasttime;
                if (($currenttime >= $time_difference) && ($currenttime <= $lasttime)) {
                    if (empty($trainingCourseAttendence)) {
                        $extacrtKey[] = $key;
                        $values[] = $query[$key];
                        $count++;
                    }
    ...
    public function pendingTrainingAttendenceClose() {
        //Configure::write('debug',2);
        $emp_code = $_SESSION['Auth']['MyProfile']['emp_code'];
        $comp_code = $_SESSION['Auth']['MyProfile']['comp_code'];
        App::import("Model", "TrainingCreation");
        ...
        $query = $model->find('all', array(
            ...
            'joins' => array(
                array(
                    'table' => 'training_course_attendence',
                    'alias' => 'TrainingCourseAttendence',
                    'type' => 'inner',
                    'foreignKey' => false,
                    'conditions' => array('TrainingCourseAttendence.training_creation_id = TrainingCreation.training_id', 'TrainingCourseAttendence.trainee_code' => $emp_code)
                )
            ),
            'conditions' => array('TrainingScheduleCreation.sch_end_date' => date('Y-m-d'),'TrainingScheduleCreation.status !=' => 1, 'TrainingCreation.status' => 5, 'TrainingCourseAttendence.open' => 1, 'TrainingCourseAttendence.close' => 0),
        ));
        ...
        if (!empty($query)) {
            ...
            foreach ($query as $key => $value) {
                ...
                $currenttime = date('Y-m-d H:i:s');
                $time = $value['TrainingScheduleCreation']['sch_end_date'] . " " . $value['TrainingScheduleCreation']['sch_end_time'];
                $time_diff = $trainingConfig['TrainingConfig']['close_attendance_hour'] . "H" . $trainingConfig['TrainingConfig']['close_attendance_min'] . "M";
                $date = new DateTime($time);
                $endtime = $date->format('Y-m-d H:i:s');
                $date->add(new DateInterval("PT$time_diff"));
                $time_difference = $date->format('Y-m-d H:i:s');
                $time_difference1[] = $date->format('Y-m-d H:i:s');
                $times[] = $endtime;

                if (($currenttime >= $endtime) && ($currenttime <= $time_difference)) {
                    $extacrtKey[] = $key;
                    $values[] = $query[$key];
                    $count++;
                }
    ...
    public function pendingTemp() {
        $emp_code = $_SESSION['Auth']['MyProfile']['emp_code'];
        App::import("Model", "TemporaryComponentDetail");
        $model = new TemporaryComponentDetail();
        $val = $model->find('count', array(
            'conditions' => array(
                'emp_code' => $emp_code,
                'status' => 2)
    ...
    public function pendingAttendance() {
        ...
        $val = $model->find('count', array(
            'conditions' => array(
                'AttendanceDetail.emp_id' => $emp_code,
                'AttendanceDetail.status' => 2)));

        return $val;
    }

    public function sanctionAttendance() {
        $emp_code = $_SESSION['Auth']['MyProfile']['emp_code'];
        App::import("Model", "AttendanceDetail");
        $model = new AttendanceDetail();
        ...
        $val = $model->find('count', array(
            'conditions' => array(
                ...
                'AttendanceDetail.status IN ("2","6")')));
     ...
     public function pendingRequisition() {
        ...
        $val = $model->find('count', array(
            'conditions' => array(
                'RequirementDetail.user_raised' => $emp_code,
                'RequirementDetail.status' => 2)));

        return $val;
    }
    public function sanctionRequisition() {
        //Configure::write
        $emp_code = $_SESSION['Auth']['MyProfile']['emp_code'];
        App::import("Model", "RequirementDetail");
        $model = new RequirementDetail();
        ...
        $val = $model->find('count', array(
            'conditions' => array(
                'RequirementDetail.approver_id' => $emp_code,
                'RequirementDetail.status IN ("2","6","9")')));

        
        return $val;
    }
  public function HrApproveAttendance() {
        $emp_code =$_SESSION['Auth']['MyProfile']['emp_code'];
        ...
        $val = $model->find('count', array(
            'conditions' => array(
                'AttendanceDetail.hr_code' => $emp_code,
                'AttendanceDetail.hr_approval_status' => 5)));
...
//        ));
    ...
    public function findUserDetailByEmpCode($emp_code = null) {
        ...
        $query = $model->find('first', array('fields' => array('user_name', 'email'), 'conditions' => array('UserDetail.emp_code' => $emp_code)));
        if (empty($query)) {
            return 0;
        } else {
            return $query;
...
//Check employee check who assgn kra to emplyee (Final Label)
    ...
    public function findKraRatingList() {
        ...
        $model = new KraRating();
        $query = $model->find('all', array(
            'fields' => array('KraRating.rating_scale', 'KraRating.overall_achievement'),
    ...
    public function upcomingTraining() {
        ...
        $query = $model->find('count', array(
            ...
            'conditions' => array('training_schedule_creation.sch_start_date >=' => date('Y-m-d'), 'training_schedule_creation.final_regis_date >=' => date('Y-m-d'), 'training_schedule_creation.status !=' => 1, 'TrainingCreation.status' => 5, 'TrainingEmployee.trainee_code' => $emp_code, 'TrainingEmployee.status !=' => 5),
        ));
    ...
    public function mailOfficeReq($frwdTo) {
        App::import("Model", "MailOffice");
        $model = new MailOffice();
        $val = $model->find('count', array(
            'conditions' => array(
                'notify' => '1',
                'forward_to' => $frwdTo,
    ...
    public function CAInvestigationNotify($current_status,$notify) {
        App::import("Model", "CAInvestigation");
        $model = new CAInvestigation();
        $val = $model->find('count', array(
            'conditions' => array(
