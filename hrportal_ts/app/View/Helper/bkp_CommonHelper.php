...
class CommonHelper extends AppHelper {
    ...
    public function findLevel($emp_code = null) {
        $comp_code = $_SESSION['Auth']['MyProfile']['comp_code'];
...
//            die('herer');
    ...
    function getHrList($emp_code) {
        ...
        $query = $model->find('list', array('fields' => array(
                'emp_code', 'emp_name'),
            'conditions' => array('dept_code' => 'DEPT00018',
                'comp_code' => $comp_code,
                'emp_code !=' . $emp_code,
                'status' => 32),
    ...
    public function getdepartmentbyid($id) {
        ...
        return $tv['Departments'][0]['dept_name'];
    }
    ...
    public function countPendingLeave($emp_code, $leave_code, $leave_id = '""') {
        ...
        //echo $emp_code;
        //echo $leave_code;
        // print_r($dummyPenleavecount); die;
    ...
    public function findAllVehical() {
...
//print_r($temp_comp_month);die;
    ...
    public function findLocationNameByCode($locationCode, $comp_code) {
        App::import("Model", "OptionAttribute");
        $model = new OptionAttribute();
        $query = $model->find('first', array('fields' => array('OptionAttribute.name'), 'conditions' => array('OptionAttribute.id' => $locationCode, 'OptionAttribute.options_id' => 3, 'OptionAttribute.ho_org_id' => $comp_code)));
 ...
 Configure::write('debug', 2);
        ...
        $query = $model->query('SELECT `MyProfile`.`emp_code`, CONCAT(`MyProfile`.`emp_full_name`, " -> ", `MyProfile`.`emp_id`) AS emp_full_name FROM `stl`.`myprofile` AS `MyProfile` LEFT JOIN `stl`.`option_attribute` AS `OptionAttribute` ON (`MyProfile`.`location_code` = `OptionAttribute`.`id`) WHERE `MyProfile`.`desg_code` = "'.$desgCode.'" AND `MyProfile`.`dept_code` = "'.$deptCode.'" ORDER BY `MyProfile`.`emp_full_name` ASC');
		//print_r($query);die;
        if (empty($query)) {
            return 0;
        } else {
	...
	function findCompetencyRatList() {
       ...
       /*  App::import("Model", "CompetencyRating");
        ...
        $result = $model->find('list', array('fields' => array('rating_scale'), 'order' => array('id ASC'), 'conditions' => array('status' => "1"))); */
    ...
    public function getKraTargetAllApprovedScoreForAppraiser($empCode, $financialYear) {
        ...
        $allScores = $model->find('all', array(
            ...
            'conditions' => array("emp_code = '$empCode'", 'KraTarget.financial_year' => "$financialYear",
                ...
                'self_score_actual != ' => '',
                'self_score_achiev != ' => '', 'YEAR(KraTarget.created_date)' => $currentYear,
                'appraiser_score_achiev != ' => '')
        ));
        return $allScores;
    }
    ...
    public function getKraTargetOpenStatusForReviewer($empCode, $financialYear) {
        ...
        $statusList = $model->find('all', array(
            ...
            'conditions' => array('KraTarget.financial_year' => "$financialYear",
                "reviewer_id" => "$reviewerCode",
                "appraiser_status = '5'", "emp_status = '2'", "reviewer_status" => array(0, 3))
        ));
        return $statusList;
    }
    ...
    public function findKraRatingList() {
	 ...
	 public function findOverAllRatingList() {
        ...
        $model = new Rating();
        $query = $model->find('all', array(
            'fields' => array('Rating.rating_name', 'Rating.rating_scale_from', 'Rating.rating_scale_to'),
    ...
    public function findGroupCompetencyDeptDesgList($GroupIds) {
	 ...
	 public function getCompAppraiserReview($empCode,$compID, $currentFinancialYear) {
        App::import("Model", 'CompetencyTarget');
        $model = new CompetencyTarget();
    ...
    public function getTotalCompModeratorCompetencyList($empCode, $moderatorId, $financialYear) {
        ...
        $list = $model->find('all', array(
            'fields' => array('*'),
            'conditions' => array('emp_code' => "$empCode",
                'moderator_id' => "$moderatorId", 'moderator_rating != ""', 'moderator_comment != ""',
                'CompetencyTarget.financial_year' => "$financialYear")
        ));
        return $list;
    }
    ...
    public function checkTrainingEvolution($training_id) {
        App::import('Model', 'TrainingMatrixAssignEmployeeAatrix');
