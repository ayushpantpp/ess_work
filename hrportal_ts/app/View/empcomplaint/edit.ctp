...
?>
<h2 class="demoheaders">Edit Complaint #<?php echo $this->data['Empcomplaint']['vc_complain_no']; ?></h2>
    ...
    <div class="input-boxs-timesheet">
            <table width="100%" border="0" cellspacing="1" cellpadding="5" class="exp-voucher innerheader">
                ...
                <tr class="cont1">
                    ...
                    <td><?php echo date("d-m-Y", strtotime($this->data['Empcomplaint']['dt_complain_date']));?><?php echo $form->hidden('vc_complain_no');?></td>
                    <th>Nature</th>
                    <td><?php echo $form->input('vc_nature',array('type'=>'select','style'=>'clear:both; margin:0px 0px 10px 0px;','options'=>array(''=>'--Select Nature--','Bug'=>'Bug','Requirement'=>'Requirement','Support'=>'Support','Wrong Data Entry'=>'Wrong Data Entry')));?></td>
                </tr>
                ...
                <tr class="cont1">
                    ...
                    <td><?php echo $form->input('vc_feedback'); ?></td>
                    <th> CC Remarks :</th>
                    <td><?php echo $form->input('vc_cc_remark'); ?></td>

                </tr>
                <tr class="cont1">
                    <th> GM/CM Remarks : </th>
                    <td><?php echo $form->input('vc_cm_remark'); ?></td>
                    <th> Man Hours :</th>
                    <td><?php echo $form->input('vc_manhours', array('readonly' => 'readonly')); ?></td>

                </tr>
                <tr class="cont1">
                    <th>Expected Closure by :</th>
                    <td><?php echo $form->input('dt_expected_closure',array('type'=>'text')); ?></td>
                    <th>Real Closure on :</th>
                    <td><?php echo $form->input('dt_real_closure',array('type'=>'text')); ?></td>
                </tr> 
                <tr class="head">
                    ...
                    <td colspan="4">
                        ...
                        <?php foreach($complain_employees as $complain_employee){?>
                            ...
                            <?php echo '<span class="tme">[ Since '.date("d-m-Y H:i:s", strtotime($complain_employee['ComplainEmployees']['dt_created'])).' ';?>
                                <input type="hidden" name="assigntime" id="assigntime" value="<?php echo date("d-m-Y H:i", strtotime($complain_employee['ComplainEmployees']['dt_created'])); ?>">
...
<div class="submit"><input type="button" style="margin-right: 5px;" value="Reset"><input name="submit_complaint" type="button" value="Submit"></div>
<?php echo $form->end();?>
                    ...
                    <td>
                        <ul class="commnets-ul">
                            ...
                            <li class="<?php echo $zebraClass = ($zebraClass=='even')?'odd':'even'; ?>">
                                ...
                                ?>
                                <strong><?php echo ucwords(strtolower($annotatorClass->id.$annotator[0][ucwords($complaint_annotation['EmpcomplaintAnnotations']['vc_user_model']).'__name'])); ?></strong><span class="tme"><?php echo $time->relativeTime(strtotime($complaint_annotation['EmpcomplaintAnnotations']['dt_created'])); ?></span><br />
                            ...
                            <?php } ?>
                            <?php if($this->data['Empcomplaint']['vc_stage']!="Forced Closure") { ?>
                            <li class="reply-comments-li">
                                ...
                                <div class="edit-comments">
                                        ...
                                        ?>
                                            <div><?php echo $form->textarea('vc_remark',array('style'=>'width:100%;','rows'=>3));?></div>
                                            ...
                                            <?php echo $form->hidden('assigningtime',array('value'=>date("d-m-Y H:i", strtotime($complain_employee['ComplainEmployees']['dt_created']))));?>
                                            <?php $options = array(
                                                            'label' => 'Post Comment',
                                                             'name' => 'post_comment',
...
<script type="text/javascript" >
    ...
    jQuery(document).ready(function(){
        jQuery('input[name=submit_complaint]').click(function(){
            var start_actual_time = $("#EmpcomplaintDtRealClosure").val();
            //alert(start_actual_time);
            var end_actual_time = $("#assigntime").val();
            //alert(end_actual_time);
            var matchDate = /(\d{2})-(\d{2})-(\d{4}) (\d{2}):(\d{2})/;
            //alert(matchDate);
            var firstDateParsed = matchDate.exec(start_actual_time);
            //alert(firstDateParsed);
            var secondDateParsed = matchDate.exec(end_actual_time);
            //alert(secondDateParsed);
            var a = new Date(firstDateParsed[3], firstDateParsed[2], firstDateParsed[1], firstDateParsed[4], firstDateParsed[5], 0);
            //alert(firstDateParsed[3]);
            var b = new Date(secondDateParsed[3], secondDateParsed[2], secondDateParsed[1], secondDateParsed[4], secondDateParsed[5], 0);
            if(a.getTime() < b.getTime()){
                alert("Real Closure on date must be greater than Engineers Assigned date.");
                return false;
            }
        ...
        jQuery('input[id^=EmpcomplaintDt]').datetimepicker({dateFormat:'dd-mm-yy'});
...
$('input[id=EmpcomplaintDtRealClosure]').change(function() {
var start_actual_time = $("#EmpcomplaintDtRealClosure").val();
//alert(start_actual_time);
var end_actual_time = $("#assigntime").val();
//alert(end_actual_time);
...
minute  = (differenceInMilliseconds / 1000 / 60 );
//alert(minute);
Hours = Math.floor(minute/60);
//alert(Hours);
Minutes = minute%60;
$('#EmpcomplaintVcManhours').val();
$('#EmpcomplaintVcManhours').val(Hours+":"+Minutes);
 });

 $('input[name=post_comment]').click(function() {
     if($('#EmpcomplaintAnnotationsVcStatus').val()=='User Feedback Awaited' || $('#EmpcomplaintAnnotationsVcStatus').val()=='Forced Closure') {
       
       function js_dd_mm_yyyy_hh_mm_ss () {
  now = new Date();
...
//alert(js_dd_mm_yyyy_hh_mm_ss());

var start_actual_time = js_dd_mm_yyyy_hh_mm_ss();
//alert(start_actual_time);
var end_actual_time = $("#EmpcomplaintAnnotationsAssigningtime").val();
//alert(end_actual_time);
...
$('#EmpcomplaintVcManhours').val();
$('#EmpcomplaintVcManhours').val(Hours+":"+Minutes);
$('#EmpcomplaintDtRealClosure').val(start_actual_time);
     }
